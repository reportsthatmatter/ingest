...
type Line = { page: number; line: number };

type NoteBlock = {
  start: number;
  end: number; // exclusive
  notes: Array<{ number: number; text: string }>;
};

/** Where the right-hand column begins, if the block has one. */
function rightColumn(lines: string[], numberColumn: number): number | null {
  const starts: number[] = [];
  for (const line of lines) {
    for (const match of line.matchAll(RIGHT_START)) {
      const at = match.index! + match[1].length + match[2].length;
      if (at > numberColumn + 20) starts.push(at);
    }
  }
 ...
 * indented past it.
 */
function splitAt(line: string, column: number): [string, string] {
  const gap = /\s{2,}/g;
...
function readColumn(
  lines: string[],
  carry?: { number: number; text: string }
): Array<{ number: number; text: string }> | null {
  const notes: Array<{ number: number; text: string }> = carry ? [carry] : [];
  for (const line of lines) {
 ...
 */
function readBlock(lines: string[], start: number): NoteBlock | null {
  const first = lines[start].match(NOTE_START);
  if (!first || first[2] !== "1") return null;
 ...
 */
function findMarker(text: string, n: number, before: number): { index: number; length: number } | null {
  const pattern = new RegExp(
    `(?<=[A-Za-z\\u00C0-\\u017F.,;:!?’”"')\\]]|\\b(?:19|20)\\d\\d)( ?)(${n})(?![\\d.]\\d|\\d)(?=[\\s.,;:)\\]’”"]|$)`,
    "g"
  );
  let found: { index: number; length: number } | null = null;
  for (const match of text.matchAll(pattern)) {
    const index = match.index! + match[1].length;
    if (index + match[2].length > before) break;
    // A space before the number is only allowed after closing punctuation
    // or a quote: "practice.” 7", never "Day 7" or "para 7".
    if (match[1] && !/[.,;:!?’”"')\]]$/.test(text.slice(0, match.index))) continue;
 ...
 */
export function extractParagraphNotes(
  pages: Array<{ body: string[]; index: number; volume?: number; pdfIndex?: number; printed?: number | null }>,
  firstBlock = 1
): { notes: Footnote[]; nextBlock: number } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let k = confirmed.notes.length - 1; k >= 0; k--) {
      ...
      for (let j = limitLine; j >= floor; j--) {
        ...
        linked.set(
          j,
          text.slice(0, marker.index) + `[^${labels[k]}]` + text.slice(marker.index + marker.length)
        );
        limitLine = j;
    ...
    for (const [j, text] of linked) lines[j] = text;
    ...
    for (let j = i; j < confirmed.end; j++) remove.add(j);
  ...
  const kept = pages.map(() => [] as string[]);
  order.forEach(({ page }, j) => {
    if (!remove.has(j)) kept[page].push(lines[j]);
  });
