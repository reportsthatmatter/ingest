 ...
 */
function isDisplacedFirstLine(lines: string[], at: number): boolean {
  ...
  if (head.length > DISPLACED_OPENING_MAX) return false;

  const inset = indentOf(above) - indentOf(opening);
  if (inset < 1 || inset > head.length + 1) return false;
 ...
 */
function runOverStart(lines: string[], at: number): number {
  let top = at;
  ...
  if (gap < RUN_OVER_MIN_GAP && at - top < 2) return at;
...
function isDoubleSpaced(lines: string[]): boolean {
  ...
  if (text < DOUBLE_SPACED_MIN_LINES) return false;
  let followed = 0;
  for (let i = 0; i < lines.length - 1; i++) {
    if (lines[i].trim() && !lines[i + 1].trim()) followed += 1;
  }
