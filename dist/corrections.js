...
function inScope(block, where) {
    if (!where)
        return true;
    if (where.volume !== undefined && block.at?.volume !== where.volume)
        return false;
    if (where.printed !== undefined && block.at?.printed !== where.printed)
        return false;
    return true;
}
 ...
 */
export function applyCorrections(blocks, corrections, reportId) {
    if (!corrections.length)
        return { blocks, applied: 0 };
