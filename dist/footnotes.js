...
export function parseFootnotes(lines, page) {
    ...
    while (i < tokens.length) {
        ...
        const nextNumbered = tokens
            .slice(i + 1)
            .find((t) => t.kind === "inline" || t.kind === "stacked");
        const outOfSequence = previous !== undefined &&
            "number" in token &&
        ...
        if ((token.kind === "inline" &&
            ...
            if (token.kind === "inline" && token.number === previous.number)
                append(token.text);
            else if (!(token.kind === "stacked" && token.number === previous.number))
                append(raw[i]);
            i += 1;
            continue;
        }
...
const DATE_BEFORE = /\b(?:January|February|March|April|May|June|July|August|September|October|November|December|Jan\.|Feb\.|Mar\.|Apr\.|Jun\.|Jul\.|Aug\.|Sept?\.|Oct\.|Nov\.|Dec\.)\s\d{1,2},$/;
...
export function linkInlineMarkers(text, known) {
    return text.replace(/([.,;:!?"'\)])\s+(\d{1,4})(?=\s|$)/g, (whole, punctuation, digits, offset) => {
        ...
        if (MONTH_ABBREVIATION_BEFORE.test(text.slice(Math.max(0, offset - 6), offset + 1)))
            return whole;
        ...
        if (NUMBER_LIST_BEFORE.test(before) &&
