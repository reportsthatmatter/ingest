...
import { contentHash, manifestFor, tokenFor } from "../publish.js";
...
if (!reportId || reportId.startsWith("--")) {
    console.error("Usage: rtm-publish <report-id> [--file full.md] [--base <url>] [--status] [--rollback <hash>]");
    process.exit(2);
}
...
const base = typeof flag("--base") === "string" ? flag("--base") : "https://reportsthatmatter.org";
...
if (typeof rollbackTo !== "string") {
    const filePath = typeof flag("--file") === "string" ? flag("--file") : "full.md";
    ...
    for (const file of files) {
        ...
        if (batch.length && size + length > MAX_BATCH)
            await flush();
        batch.push(file);
        size += length;
    }
