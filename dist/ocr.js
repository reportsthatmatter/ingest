...
export function findSuspects(text, page) {
    const suspects = [];
    for (const [pattern, label, confidence] of SUSPECT) {
        for (const match of text.matchAll(pattern)) {
            const value = match[0];
            if (DICTIONARY_SAFE.has(value.toLowerCase()))
                continue;
...
export function rankSuspects(suspects) {
    ...
    for (const suspect of suspects) {
        ...
        if (existing)
            ...
            byMatch.set(key, { ...suspect, count: 1 });
    }
    return [...byMatch.values()].sort((a, b) => order[a.confidence] - order[b.confidence] || b.count - a.count);
}
