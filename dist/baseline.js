...
export function computeBaseline(result, poppler) {
    ...
    const blocks = {
        ...
        paragraph: lines.filter((line) => line.trim() && !/^([#>\-]|%%page|\[\^)/.test(line.trim())).length,
    };
    return {
...
export function diffBaselines(before, after) {
    ...
    if (before.poppler !== after.poppler) {
        out.push(`poppler ${before.poppler} → ${after.poppler} — tool drift, not a code ` +
    ...
    for (const kind of Object.keys({ ...before.blocks, ...after.blocks })) {
        ...
        if (a !== b)
            out.push(`${kind}: ${a} → ${b} (${b > a ? "+" : ""}${b - a})`);
    }
    for (const field of ["words", "footnotes", "pageMarkers"]) {
    ...
    for (const heading of gone.slice(0, 10))
        out.push(`heading removed: ${heading}`);
    for (const heading of added.slice(0, 10))
        out.push(`heading added:   ${heading}`);
    if (gone.length > 10)
        out.push(`… and ${gone.length - 10} more headings removed`);
    if (added.length > 10)
        out.push(`… and ${added.length - 10} more headings added`);
