...
export function parseContentsPage(lines) {
    ...
    for (const line of lines) {
        ...
        buffer = [];
        if (text)
            blocks.push({ kind: "contents", text, page: leaders[1] });
    }
 ...
 */
export function bodyIndent(lines) {
 ...
 */
const DANGLING = /\b(of|the|a|an|in|to|for|and|or|with|by|from|that|was|is|are|were|on|at|as|its|their|his|her)$/i;
/**
 ...
 */
const STOPWORD = /^(of|the|a|an|in|to|for|and|or|nor|with|by|from|that|as|at|on|is|was|were|are|be|been|its?|it|their|his|her|our|my|your|has|have|had|but|not|no|than|then|so|if|when|which|who|whom|whose|into|upon|per|via)$/i;
/**
...
export function danglesMidPhrase(text) {
 ...
 */
export function isTabularPage(lines) {
...
function isHeadingLine(text, allowDivisions = true, allowAllCaps = true, allowNumbered = true) {
    ...
    if (TOC_ENTRY.test(trimmed))
        return null;
    const body = stripTrailingPageNumber(trimmed);
    if (!body || body.split(/\s+/).length > HEADING_MAX_WORDS)
        return null;
    if (/[;:,]$/.test(body))
        return null;
 ...
 */
function joinParagraphContents(lines) {
    const out = [];
    const nextFilled = (i) => {
 ...
 */
export function contentsHeadings(blocks) {
    ...
    for (const block of blocks) {
        ...
        if (/^\d{1,2}\.\d{1,3}$/.test(block.page))
            subsections.add(block.text);
    }
    if (!entries.size)
        return blocks;
    const out = [];
    for (let i = 0; i < blocks.length; i++) {
        ...
        if (block.kind === "paragraph") {
            const title = [...subsections].find((t) => block.text.endsWith(` ${t}`) && /[.!?"”]$/.test(block.text.slice(0, -t.length - 1)));
            if (title) {
...
export function toBlocks(lines, documentMargin, quoteInset = DEFAULT_QUOTE_INSET, numberedParagraphs = false, allCapsHeadings = true, paragraphContents = false, numberedHeadings = true, listed) {
    ...
    const DIVISION_LINE = /^(\s*)(?:Part|Chapter|Appendix|Annex|Volume|Section)\s+(?:\d{1,3}|[IVXLC]{1,7}):?\s+/;
    ...
    for (const [i, line] of lines.entries()) {
        ...
        if (list) {
            ...
            if (inBulletRun[i] && !structural[i] && indentOf(line) >= listTextIndent - 1) {
                list[list.length - 1] = normaliseWhitespace(`${list[list.length - 1]} ${line.trim()}`);
                continue;
            }
            list = null;
        }
...
export function endsSentence(text) {
    ...
    if (!/[.?!:;]$/.test(terminal))
        return false;
    if (ABBREVIATION.test(text) || INITIAL.test(text))
        return false;
    return true;
}
